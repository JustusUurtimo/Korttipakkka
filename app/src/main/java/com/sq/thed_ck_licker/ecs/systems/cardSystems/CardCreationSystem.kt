...
class CardCreationSystem @Inject constructor(
    ...
    fun addMaxHpTrapCards(amount: Int = 5): List<EntityId> {
        ...
        val maxHp = 10f
        val onActivation = { targetId: Int ->
            val latestCardEntity = (targetId get LatestCardComponent::class).getLatestCard()
            val targetHp = targetId get HealthComponent::class
            if (MyRandom.getRandomInt() <= 1) {
                (latestCardEntity get HealthComponent::class).damage(cardHealthAmount)
        ...
        val describedEffect = DescribedEffect(onActivation) { targetId: Int ->
            val targetHp = targetId get HealthComponent::class
            val damageAmount = abs(targetHp.getHealth().div(2))
            "Gain $maxHp max health on play, might explode ($damageAmount damage)"
        }
    ...
    fun addBreakingDefaultCards1(amount: Int = 7): List<EntityId> {
    ...
    fun addBreakingDefaultCards(amount: Int = 7): List<EntityId> =
        cardBuilder.addBreakingDefaultCards(amount)
    ...
    fun addCombiningTestCards(numberOfCards: Int = 1, parts: List<EntityId>): List<EntityId> {

        var hp = 1f
        val effects = mutableListOf<DescribedEffect>()
        parts.forEach { part ->
            val effect = part get EffectComponent::class
            effects.addAll(effect.getNonEmptyEffects())
        }
        while (effects.size < 6) {
            effects.add(DescribedEffect.EMPTY)
        }
        ...
        val dd = EffectComponent(
            onDeath = effects[0],
            onDrawn = effects[1],
            onTurnStart = effects[2],
            onPlay = effects[3],
            onDeactivate = effects[4],
            onSpecial = effects[5]
        )
        ...
        cards.applyComponentToAll(dd)
        return cards
    }

    fun ggg(): (Int) -> Unit {
        val thingthing: (Int) -> Unit = { targetId: EntityId ->
            val thhh = (targetId get DrawDeckComponent::class).getDrawCardDeck()
            val thhh2 = (targetId get DiscardDeckComponent::class).getDiscardDeck()
            val combo = mutableListOf<EntityId>()
            combo.addAll(thhh)
            combo.addAll(thhh2)
            combo.shuffle()
            val eka = combo.first()
            val toka = combo.last()
            (targetId get DrawDeckComponent::class).removeCards(listOf(eka, toka))
            (targetId get DiscardDeckComponent::class).removeCards(listOf(eka, toka))
            val efff: MutableList<DescribedEffect> =
                ((eka get EffectComponent::class).getNonEmptyEffects() + (toka get EffectComponent::class).getNonEmptyEffects()) as MutableList
